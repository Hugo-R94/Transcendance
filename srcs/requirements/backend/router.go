...
import (
	...
	"github.com/Hugo-R94/Transcendance/backend/internal/apiHandlers/support"
...
func setupRouter(db *gorm.DB, hub *chat.Hub) *gin.Engine {
	...
	if trustedProxies[0] == "" {
		...
		log.Printf("[INFO] No TRUSTED_PROXIES detected in env, using default 127.0.0.1")
	}
	router.SetTrustedProxies(trustedProxies)
	allowedOrigin := os.Getenv("FRONTEND_ORIGIN")
	if allowedOrigin == "" {
		allowedOrigin = "https://localhost:8443"
		log.Printf("[INFO] No FRONTEND_ORIGIN in env, using default %s", allowedOrigin)
	}
	...
	user.SetUserInfo(userGroup, v1, db)
