...
function LikeButton({
  ...
  const [isPending, setIsPending] = useState<boolean>(false);
	...
	let newState = targetVote;
	let newLikes = likes;
	let newDislikes = dislikes;

	// Même bouton => toggle visuel
	...
	// Changement like <-> dislike => affichage
	else {
		if (userState === 1) {
		newLikes = Math.max(0, likes - 1);
		}
	...
	try {
		// IMPORTANT :
		// Le backend reçoit TOUJOURS le vote demandé.
		...
		// Rollback uniquement de l'affichage
  ...
  return (
    <div className="w-50 h-10 rounded-2xl flex m-auto">
      ...
      >
        {userState === 1 ? (
          <svg
            ...
            viewBox="0 0 24 24"
            fill="currentColor"
            className="size-6 text-green-400 shrink-0"
          >
        ...
        <p className="mx-auto font-bold text-white">{likes}</p>
      </button>
      ...
      <button
        ...
        disabled={isPending}
        onClick={() => handleVote(-1)}
        className={`flex justify-center shadow-md shadow-black/50 hover:shadow-lg hover:translate-x-3 hover:outline-2 balatro active:scale-90 items-center gap-2 bg-black/30 w-1/2 h-full p-1 rounded-r-2xl transition-all ${
          userState === -1 ? "bg-red-600/40 border-2 border-red-500" : ""
        }`}
      >
        {userState === -1 ? (
          <svg
        ...
        <p className="mx-auto font-bold text-white">{dislikes}</p>
      </button>
    </div>
  );
}
