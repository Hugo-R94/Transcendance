...
export function useChat() {
  ...
  const fetchMyProfile = async () => {
    try {
      ...
      console.error("[CHAT] Erreur profil :", err.response?.data || err.message);
    }
  };
  ...
  const fetchConversations = async () => {
    try {
      ...
      console.error("[CHAT] Erreur conversations :", err.response?.data || err.message);
    }
  };
  ...
  const getWebSocketUrl = (token: string) => {
    ...
    try {
      ...
      const url = new URL(`${wsProtocol}//${base.host}${base.pathname.replace(/\/$/, "")}/ws`);
  ...
  useEffect(() => {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (data.type === "message") {
            setConvs((prev) => prev.map((conv) => String(conv.id) !== String(data.conversation_id) ? conv : { ...conv, messages: [data, ...(conv.messages || [])] }));
            fetchConversations();
        ...
        } catch (err) { console.error("[CHAT] JSON invalide :", err); }
      };
    ...
    return () => {
      ...
      wsRef.current = null;
      if (ws && (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING)) ws.close(1000, "unmounted");
  ...
  const getOtherUser = (conv: Conversation) => {
    ...
    if (String(conv.user1_id) === String(myUserId)) return conv.user2;
  ...
  const handleFriendClick = async (friend: Friend) => {
    const conversation = convs.find((conv) => {
      ...
      return other && String(other.id) === String(friend.id);
    });
    ...
    if (!openConvIds.includes(String(conversation.id))) {
      setOpenConvIds((prev) => [...prev, String(conversation.id)]);
    }

    const myUserId = localStorage.getItem("userID");
    const isUser1 = String(conversation.user1_id) === String(myUserId);
    const isUser2 = String(conversation.user2_id) === String(myUserId);
    if (!isUser1 && !isUser2) return;

    try {
      await api.put(`/convs/${conversation.id}/read`);
      setConvs((prev) => prev.map((conv) => {
        if (String(conv.id) !== String(conversation.id)) return conv;
        return isUser1 ? { ...conv, user1_a_jour: true } : { ...conv, user2_a_jour: true };
      }));
    } catch (err: any) {
      console.error("[CHAT] Erreur lecture :", err.response?.data || err.message);
    }
  };

  const handleSendFriendRequest = async () => {
    const username = targetUsername.trim();
    if (!username) { showNotification("Entrez un nom d'utilisateur."); return; }
    try {
